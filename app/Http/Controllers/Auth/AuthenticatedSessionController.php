...
class AuthenticatedSessionController extends Controller
{
    ...
    public function store(LoginRequest $request)
    {
        ...
        $user = \Auth::user();
        if ($userData['modules']['name'] == 'Company' ) {
        ...
        if ($userData['modules']['name'] == 'Company' ) {
            ...
            if ($user->plan != $free_plan->id) {
                if (date('Y-m-d') > $user->plan_expire_date && $plan->duration != 'Lifetime') {
                    ...
                    $users     = User::where('created_by', '=', $userData['id'])->get();
        ...
        if ($user->type != 'company' && $user->type != 'Super Admin') {
            ...
            $login_detail->user_id = $userData['id'];
            $login_detail->ip = $ip;
            $login_detail->date = date('Y-m-d H:i:s');
            $login_detail->Details = $json;
            $login_detail->created_by = $userData['id'];
            $login_detail->save();
        }
    ...
    public function showLoginForm($lang = '')
    {
        ...
        return view('auth.login', compact('userData','lang'));
    }
    ...
    public function showLinkRequestForm($lang = '')
    {
        ...
        return view('auth.forgot-password', compact('userData','lang'));
    }
